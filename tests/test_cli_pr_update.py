...
def test_pr_update_patches_description_from_body_file(monkeypatch, tmp_path: Path) -> None:
    runner = CliRunner()
    config = make_config()
    patched: list = []
    ...
    result = runner.invoke(pr_mod.app, ["update", "--body-file", str(body_file)])

    assert result.exit_code == 0, result.output
    assert patched == [{"owner": "sam", "repo": "joan", "index": 3, "body": "Body from file"}]
