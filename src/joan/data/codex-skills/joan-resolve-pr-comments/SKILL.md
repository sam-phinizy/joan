...
1. Verify `.joan/config.toml` exists. If not, tell the user to run `$joan-setup`
   first.
...
If the user points you at a different PR, or you are not on the branch whose PR you need to inspect, use:
...
Prefer `--branch` when you know the latest review branch you need, but you are not currently checked out on it.
...
- whether the user can now move on to pushing upstream with `$joan-review` or
  `uv run joan pr push`
